...
puts '-*-*-*-*-*-*-*-*-*-*-*-*-'
puts 'my_any??'
p [12,3,34].my_any?(Integer) #=> true
p ['ghetto','wild','zoo'].my_any?(/z/) #=>true
...
p [23,13,12,43].my_count(&block) #1
ary = [1, 2, 4, 2]
puts ary.my_count #=> 4
...
puts [1, 2, 3].my_inject(:+)  #should return 6
puts [1, 2, 3].my_inject(1, :+)  #should return 7
puts '-*-*-*-*-*-*-*-*-*-*-*-*-'
puts 'multiply_els'
